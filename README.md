...
**A Liquid Glass status HUD for your Mac.** GlassBar floats a single translucent capsule at the top of your screen that shows — at a glance — **what's running** across your AI / dev tools, **how much of your Claude & Codex plan you've used**, and **what's playing**, without ever touching your Dock or menu bar. Drag it anywhere and it remembers. When a session finishes, a **pixel‑art crab scuttles across your screen** and you get a notification.

![GlassBar bar](docs/screenshot.png)

## Highlights

- **Live AI session tracking** — Claude Code & Codex session counts, real plan limits, and per‑session token usage.
- **☕ Keep awake** — close your laptop lid without the Mac sleeping, so long‑running sessions keep going (works on **AC *and* battery**).
- **Now playing + transport** — see and control the current track (⏮ ▶/⏸ ⏭) right from the bar.
- **Movable & persistent** — drag the bar anywhere; its position is restored on launch.
- **Finish alerts + mascots** — a hand‑drawn pixel crab announces completed sessions, with a tap‑to‑focus notification.
- **100% local** — reads your own files and account; no third‑party services.

## The bar

Left → right:

- **App logos** — VS Code · Cursor · Claude desktop · ChatGPT. Full‑color when running, dimmed when not. **Click to focus** (or launch if it's closed).
...
- **♪ Now playing** — the current track (or the app making sound, or *Not playing*), with **⏮ previous · ▶/⏸ play‑pause · ⏭ next** controls.
- **☕ Keep awake** — toggle to keep the Mac awake with the lid closed; glows **orange** when active ([details below](#keep-your-sessions-alive-with-the-lid-closed)).
...
## Keep your sessions alive with the lid closed

The **☕ coffee‑cup** toggle lets you **close your laptop screen without the Mac going to sleep**, so running Claude Code / Codex sessions keep working. The icon glows **orange** while active, and the setting is remembered across launches.

| Power source | How it stays awake | Prompt |
|---|---|---|
| **AC (plugged in)** | In‑process power assertion — same mechanism as `caffeinate -s` | none |
| **Battery** | Additionally sets `pmset disablesleep` system‑wide | admin password, once |

- **The screen always turns off when the lid closes — that's expected.** Only the *system* stays awake underneath, so your sessions keep running.
- **To restore normal sleep, toggle it off** (asks for your password again on battery).
- Quitting GlassBar while it's on does **not** auto‑revert the battery setting — turn the toggle off, or run `sudo pmset -a disablesleep 0`.
- ⚠️ Running real compute in a **closed laptop on battery** generates heat — don't leave it sealed in a bag.
...
`build.sh` compiles a release binary, assembles `GlassBar.app`, installs it to `~/Applications`, registers a **login LaunchAgent** (auto‑start), clears any old instance, and launches it.

## Update & uninstall

Re‑run `./build.sh` any time to rebuild and relaunch with the latest changes. To remove GlassBar completely:
